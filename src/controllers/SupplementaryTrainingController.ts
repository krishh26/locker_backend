...
export class SupplementaryTrainingController {
    ...
    // Admin: Export learner feedback rows as CSV (scoped; optional search)
    public async exportFeedbacksCsv(req: CustomRequest, res: Response) {
        try {
            ...
            const filename = `supplementary-training-feedbacks-${dateStamp}.csv`;
    ...
    public async deleteResource(req: CustomRequest, res: Response) {
        try {
            ...
            await actRepo
                .createQueryBuilder()
                .delete()
                .from(LearnerSupplementaryTrainingActivity)
